...
public class BuildMapper {
    ...
    public static void execute(TableInfo tableInfo) {
        ...
        try {
            ...
            if (splitTable) {
                bw.write("public interface " + tableInfo.getBeanName() + Constants.SUFFIX_MAPPER + "<T,P> extends BaseMapperTableSplit<T,P> {");
            } else {
            ...
            for (Map.Entry<String, List<FieldInfo>> entry : keyMap.entrySet()) {
                ...
                if (splitTable) {
                    tableParams = "@Param(\"tableName\") String tableName,";
                }
                if (paramStr.length() > 0) {
                    BuildComment.buildMethodComment(bw, "根据" + methodName + "更新");
                    bw.newLine();
                    bw.write("\t Integer updateBy" + methodName.toString() + "(" + tableParams + "@Param(\"bean\") T t," + paramStr.toString
                            () + ")" + ";");
                    bw.newLine();
                    bw.newLine();
                    ...
                    bw.newLine();
                    bw.write("\t Integer deleteBy" + methodName.toString() + "(" + tableParams + paramStr.toString() + ");");
                    bw.newLine();
                    bw.newLine();
                    BuildComment.buildMethodComment(bw, "根据" + methodName + "获取对象");
                    bw.newLine();
                    bw.write("\t T selectBy" + methodName.toString() + "(" + tableParams + paramStr.toString() + ");");
                    bw.newLine();
                    bw.newLine();
                }
            }
